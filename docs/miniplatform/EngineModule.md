...
3. 测试列表及测试用例的装载
4. 测试用例的生命周期管理和运行   
   测试引擎与测试用例是强绑定关系，测试引擎负责测试用例的执行过程，测试引擎执行测试用例的最小单位是一个测试列表。   


testengine/caserunner.py
```python
"""
Test Engine
"""
...
if __name__ =="__main__":
    load_settings("settingfile")
    init_engine()
    load_resource("D:/Code/TestProject/automation_test/product/resource/test.json", "tester")
    load_test_list("D:/Code/TestProject/automation_test/product//testlist/demo_list.testlist")
    runner.start()
    runner.wait_for_test_done()
    print(runner.result_report.root.to_text())
```
...
### 最终执行命令
python3 start.py -s settingfile -t ../example/demo_list.testlist -r ../example/resource.json -u tester